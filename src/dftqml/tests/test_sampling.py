...
class TestFhchain(unittest.TestCase):
    ...
    def test_dftio(self):
        target_density, target_energy = self.sampler.system.dftio(
            self.sampler.ground_state)
        sampled_density, sampled_energy = self.sampler.dftio(
            self.nshots, self.nshots)
        sampled_density, sampled_energy = self.sampler.dftio(
            self.nshots, self.nshots)
    ...
    def test_coulomb_sampling(self):
        coulomb_fop = of.transforms.reorder(
            of.fermi_hubbard(self.sampler.system.nsites, 1, 0,
                             self.sampler.system.coulomb), of.up_then_down)
        ...
        sampled_coulomb = self.sampler.average_coulomb_from_cb_samples(
            cb_samples)

        coulomb_relative_error = np.abs(
            sampled_coulomb - target_coulomb) / target_coulomb
