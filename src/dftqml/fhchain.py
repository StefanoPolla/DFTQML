...
def projector_from_block_to_singlet(
        nsites: int, block_indices: ArrayLike, up_then_down: bool = True) -> ArrayLike:
    s2_fop = of.hamiltonians.s_squared_operator(nsites)
    if up_then_down:
        s2_fop = of.transforms.reorder(s2_fop, of.up_then_down)
    s2_block = of.get_sparse_operator(s2_fop, n_qubits=2 * nsites
...
def projector_block_and_singlet(
        nsites: int, nelec: int, up_then_down: bool = True) -> ArrayLike:
    """
    ...
    block_projector = scipy.sparse.diags(np.ones(4**nsites), format='csc'
                                         )[block_indices, :]
...
class FermiHubbardChain():
    ...
    def __init__(self, nsites, nelec, coulomb, up_then_down=True):
        ...
        self.homogeneous_hamiltonian = of.fermi_hubbard(x_dimension=nsites,
                                                        y_dimension=1,
                                                        tunneling=1,
                                                        coulomb=coulomb,
                                                        periodic=True)
        ...
        self.block_homogeneous_hamiltonian = self.block_project(
            self.homogeneous_hamiltonian)
        # electron density regardless of spin

        if up_then_down:
            self.density = [
                of.FermionOperator(((i, 1), (i, 0),),)
                + of.FermionOperator(((i + nsites, 1), (i + nsites, 0),),)
                for i in range(nsites)]
        else:
            self.density = [
                of.FermionOperator(((i, 1), (i, 0),),)
                + of.FermionOperator(((i + 1, 1), (i + 1, 0),),)
    ...
    def block_project(self, symbolic_operator):
        return project_operator(
            of.get_sparse_operator(symbolic_operator,
                                   n_qubits=2 * self.nsites),
    ...
    def hamiltonian(self, potential=None) -> of.FermionOperator:
        if potential is None:
            ...
            return self.homogeneous_hamiltonian + np.dot(potential,
                                                         self.density)
    ...
    def block_hamiltonian(self, potential=None):
        if potential is None:
            ...
            if len(potential) != self.nsites:
                raise ValueError('potential has the wrong lenght')
            return (self.block_homogeneous_hamiltonian
                    + np.tensordot(potential, self.block_density,
                                   axes=([0], [0])))
    ...
    def homogeneous_block_gse(self):
        ...
        return scipy.sparse.linalg.eigsh(
            self.block_homogeneous_hamiltonian, k=1, which='SA')[0][0]
    ...
    def density_expectation(self, state):
        ...
        return np.real([state.conj() @ nop @ state
                        for nop in self.block_density])

    def homogeneous_energy_expectation(self, state):
        state = self._check_and_project_state(state)
        return np.real(
            state.conj() @ self.block_homogeneous_hamiltonian @ state)
    ...
    def dftio(self, state):
        ...
        '''
        state = self._check_and_project_state(state)
        return (self.density_expectation(state),
                self.homogeneous_energy_expectation(state))
